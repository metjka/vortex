...
// Assembly location: C:\Users\isalnikov\Desktop\CntProceduralTextures\CntProceduralTextures\bin\Debug\Gen.dll
    ...
    public string exeScript(string script) {
        ...
        foreach(string str1 in strArray1)
        {
            ...
            if (str2.Trim() != "" && !str2.Trim().StartsWith("//")) {
                string[] strArray2 =
                        Regex.Replace(str2.Split(new string[1] {
                    "//"
                },StringSplitOptions.None)[0].Trim(), "\\s+", " ")
                            .Split((char[]) null);
                ...
                if ((object) method == null && name.Contains("->")) {
                    string[] strArray3 = name.Split(new string[1] {
                        "->"
                    },StringSplitOptions.None);
    ...
    int flatCoord(int i, int j, int k);

    public delegate Color

    getPixelDelegate(int i, int j, float lat);

    public delegate Color

    getPixelDelegateCB(int i, int j, int k, float lat);
}
}