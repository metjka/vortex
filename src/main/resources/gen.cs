...
// Assembly location: C:\Users\isalnikov\Desktop\CntProceduralTextures\CntProceduralTextures\bin\Debug\Gen.dll
        ...
        private void addStar(int x, int y, double power, int radius){
            ...
            for (int i1 = x - num1; i1 < x + num1; ++i1){
                for (int i2 = y - num1; i2 < y + num1; ++i2)
                {
                    ...
                    int sy = this.seamlessCoord(i2, this.hei, seamlessY);
                    double val1_1 = Math.Abs(sx - x);
                    double val1_2 = Math.Abs(sy - y);
                    double val2_1 = wid - val1_1;
                    double val2_2 = hei - val1_2;
                    double num2 =
                        Math.Sqrt(Math.Pow(Math.Min(val1_1, val2_1), 2.0) + Math.Pow(Math.Min(val1_2, val2_2), 2.0));
                    if (num2 < 0.001)
                        num2 = 0.001;
                    addV(sx, sy, (float) (power * (double) radius / num2));
                }
            }
        }
        ...
        private Color getGreyScale(int i, int j, float lat)
        {
            ...
            if ((double) num > (double) byte.MaxValue)
                num = (float) byte.MaxValue;
            var c = Color.FromArgb((int) num, (int) num, (int) num);
            return c;
        }
        ...
        public Bitmap getBitmap(int startX, int endX, int startY, int endY, Gen.getPixelDelegate f)
        {
            ...
            for (int x = 0; x < w; ++x)
            {
                for (int y = 0; y < h; ++y)
                {
                    float lat = Math.Abs((float) ((double) y / ((double) this.hei / 2.0) - 1.0)) * this.Lat01Factor;
            ...
            int fromP, int toP, int fromR, int toR)
        {
            if (spiralHarms == 0)
                spiralHarms = 1;
            double num = 2.0 * Math.PI / spiralHarms;
            double startAngle = 0.0;
            ...
            double gaussianMean, double gaussianVariance, double perturbation)
        {
            ...
            double num1 = Math.PI * cicles * 2.0;
            double num2 = nextGaussian(gaussianMean, gaussianVariance) * num1;
            double num3 = a + b * num2;
            double num4 = num3 + r.NextDouble() * (num3 * perturbation);
            double num5 = startAngle + num2;
            int num6 = (int) (num4 * Math.Cos(num5));
            int num7 = (int) (num4 * Math.Sin(num5));
            numArray[0] = seamlessCoord(centerX + num6, wid, seamlessX);
            numArray[1] = seamlessCoord(centerY + num7, hei, seamlessY);
            numArray[2] = (int) num4;
            numArray[3] = (int) num2;
            return numArray;
        }
